...
from admissions.reports import *
from admissions.models import *
from admissions.forms import InquiryForm, ReportForm, TemplateReportForm
        ...
        if 'template_report' in request.POST:
            ...
            if report_form.is_valid():
                ...
                if 'applicants_to_students' in request.POST:
                    return HttpResponseRedirect(reverse_lazy(applicants_to_students, args=[year[0].id]))
                    ...
                    if year_ids:
                        year_ids = year_ids[:-1]
                    return HttpResponseRedirect(reverse_lazy(funnel) + '?year_ids=%s' % (year_ids))
...
def funnel(request):
    ...
    running_male = 0
    running_female = 0
    for level in reverse_lazyd(levels): # don't use levels.reverse_lazy() it makes a copy not reference!
...
def ajax_check_duplicate_applicant(request, fname, lname):
    ...
    for applicant in applicants:
        data += '<a href="%s" target="_blank">%s %s %s - %s</a><br/>' % (reverse_lazy('admin:admissions_applicant_change', args=(applicant.id,)), applicant.fname, applicant.mname, applicant.lname, applicant.bday)
